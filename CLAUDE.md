...
| DAERA daily prediction | `services-eu1.arcgis.com/.../Bathing_Waters_Predictive_Modelling_view/FeatureServer/0/query` | None | Crown copyright |
| Scottish council areas | `api.postcodes.io/postcodes` (build time) | None | OGL v3 |
...
`/beaches` and `/beaches/[place]` rank the cleanest bathing waters per country and per region (at least five sites), derived entirely from the catalogue (`src/lib/data/places.ts`) so they never carry stale hand-written rankings.

**An unclassified place cannot be ranked by classification.** `PlacePage.classified` says whether the regulator publishes one at all. Where it does not, `/beaches/[place]` orders by the regulator's reading instead and says so. Reading an empty `cleanest` as "no bathing water here holds an Excellent or Good classification" is the same overclaim inverted, and it is what the page said for Northern Ireland until it was caught in review.

**Scottish regions are derived, not given.** SEPA's feed carries no district, so all 90 Scottish sites used to roll up to a single country page against 29 region pages for England. `addScottishRegions` in the build reverse-geocodes each site through postcodes.io, the same open service the site already uses for postcode entry, in one bulk POST. That yields 8 Scottish region pages (Fife, Highland, East Lothian, Aberdeenshire, South Ayrshire, Dumfries and Galloway, North Ayrshire, Angus). The search radius is `POSTCODE_SEARCH_RADIUS_M` (2km): the worst genuine match across all 90 sites is 673m, and a wider radius would let an estuary site match across the water in the wrong council, since both the Forth and the Clyde are under 5km across in places. Requests are chunked at `POSTCODE_BULK_LIMIT` (100), which is postcodes.io's hard cap: unchunked, ten more Scottish designations would 400 the whole POST and silently delete every Scottish region hub on the next build. A weekly GitHub Action (`.github/workflows/refresh.yml`) pings a Vercel deploy hook (secret `VERCEL_DEPLOY_HOOK`) to rebuild and keep them current.
...
## Northern Ireland has no annual classification

DAERA publishes none. This is the single biggest difference between NI and the other three countries and it was got badly wrong once.

The DAERA feed carries `water_quality_indicator`, which was read straight into `classification`. It is not a classification: it is a **weekly judgement taken from that week's sample**, on its own vocabulary (`Excellent` / `Satisfactory` / `NoBathing`), with its own `Sampling_datetime`. Reading it as a classification made 29 pages claim a four-year percentile the regulator has never published, and dropped the 4 sites whose value `classifyValue` did not recognise into the parser's catch-all. One of those four is `NoBathing` at Rea's Wood on Lough Neagh: DAERA advising against bathing rendered as "unclassified" and produced a Caution.

So:

- NI rows carry `classification: 'Unknown'`, which is the truth, plus `currentAssessment` (`level`, the regulator's own `label`, and `assessedAt`).
- **`currentAssessment.label` is DAERA's own word.** Never rename it. The engine's internal `level` says how to treat it; the label is what the reader sees, so a site DAERA calls Excellent is not shown as "Good".
- `advised-against` returns **No, year-round**, ahead of the classification checks. It is advice against bathing, not a rating, and it is not aged out: a warning that lapses quietly is worse than one shown with its date.
- A clean reading inside `SAMPLE_CURRENT_DAYS` stands in for the missing classification, so NI sites get real verdicts instead of a permanent Caution. Past that it is a dated factor only.
- The "Annual classification" factor is suppressed where no classification is published, since "Annual classification: Unknown" is noise sitting under a verdict the regulator's own advice drove. The reading row that replaces it is added by `appendBaseFactors`, so it appears on **every** verdict path. Added in a branch instead, it vanished from a discharge-driven No, which is where standing advice against bathing most needs to be visible.
- **An undated reading counts as stale**, which is the opposite of how an undated sample is treated, and deliberately. An undated sample fails safe by continuing to drive a No; an undated clean reading would fail *open*, pinning a site to Yes for ever on a reading of unknown age.
- The 28-day window is applied on cards and share cards too (`isReadingCurrent`, and the same constant in `og-card.mjs`). Sampling stops on 15 September, so without it every NI card would read "Latest reading Excellent" all winter beside a page that had already demoted the site to Caution. Past the window they read "Last reading".
- **Never state scope from absence.** "DAERA runs a prediction for six sites, and this is not one of them" was wrong: no forecast also means a fetch failure, a withheld value, a stale day stamp or an unknown word, all of which happen to sites that are in the programme.
- `ratingLabel` shows "Latest reading Excellent" on cards rather than "Unclassified", worded to keep a single week's sample visibly apart from a multi-year percentile. `currentAssessment` therefore rides on the slim search index too, or every NI card in a nearby or postcode result would read "Unclassified" again.
- The OG share card carries it as well, with the red stripe on `advised-against`. "Not yet classified" would promise a classification that is never coming, and would put the site DAERA advises against bathing on a neutral card.
